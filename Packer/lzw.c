...
BOOL LzwCompressData(IN PBYTE pRawData, IN SIZE_T sRawDataSize, OUT PBYTE* ppCompressedData, OUT PSIZE_T psCompressedDataSize)
{
	...
	if (ssResults < 0x00) {
		//		printf("[!] lzw_compress Failed With Error: %d \n", ssResults);
		goto _FUNC_CLEANUP;
	}
...
BOOL LzwDecompressData(IN PBYTE pCompressedData, IN SIZE_T sCompressedDataSize, OUT PBYTE* ppRawData, IN OUT PSIZE_T psRawDataSize)
{
	...
	SSIZE_T					ssResults = 0x00,
		ssWritten = 0x00;
	PBYTE					pRawData = NULL;
	SIZE_T					sAllocated = ROUND_UP_TO_1024(*psRawDataSize);		// Allocate some additional extra memory 
	...
	// Checking the return value
	if (ssResults < 0x00) {
		//		printf("[!] lzw_decompress Failed With Error: %d \n", ssResults);
		goto _FUNC_CLEANUP;
	}
